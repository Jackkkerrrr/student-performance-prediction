...
def train(model, lr, lamb, train_data, zero_train_data, valid_data, num_epoch):
    ...
    for epoch in range(0, num_epoch):
        ...
        for user_id in range(num_student):
            ...
            # Compute the loss.
            # Add the regularizer.
            autoencoder_weight_norm = model.get_weight_norm()
            reg_loss = 0.5 * lamb * autoencoder_weight_norm
            loss = torch.sum((output - target) ** 2.0) + reg_loss
            loss.backward()
    ...
    plt.tight_layout()
    plt.savefig(f'training_curves_k{model.g.out_features}_lambda{lamb}.png')
    plt.close()
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
...
def main():
    ...
    for lamb in [0.001, 0.01, 0.1, 1]:
        print(f"Training AutoEncoder with lambda={lamb}...")
    ...
    print(f"Best lambda: {best_lamb} with validation accuracy: {best_acc:.4f}")
    test_acc = evaluate(best_model, zero_train_matrix, test_data)
    print(f"Test accuracy with best k={best_k}: {test_acc:.4f}")
